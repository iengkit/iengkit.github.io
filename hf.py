...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
    ...
    ssd_rows = int(tex_rows - 2 * patchL)
    ssd_cols = int(tex_cols - 2 * patchL)
    SSD = np.zeros((ssd_rows,ssd_cols))
    L = []
    assert TODOMask.shape[0] == patch_cols, "Houston, we've got a problem"
    assert TODOMask.shape[1] == patch_rows, "Houston, we've got a problem"
    assert tex_rows >= patch_rows, "Houston, we've got a problem"
    assert tex_cols >= patch_rows, "Houston, we've got a problem"
    j = -1
    for i in range(patch_rows, tex_rows+1):
        j += 1
        p = 0
        for k in range(patch_rows, tex_cols+1):
            ims = textureIm[j:i, p:k]
            newim = deepcopy(ims)
            for t in range(0, patch_rows):
                for s in range(0, patch_rows):
                    if (TODOMask[t][s] == 1):
                        newim[t][s].fill(0)
                        TODOPatch[t][s].fill(0)
            L.append(sum(sum(sum((newim-TODOPatch)**2))))
            p += 1
    La = np.asarray(L)
    La.shape = (ssd_rows,ssd_cols)
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
    ...
    dm = int((patchSize-1)/2)
    for i in range(patchSize):
        for j in range(patchSize):
                    if (TODOMask[i][j] == 1):
                        imHole[iPatchCenter-dm:iPatchCenter+dm+1, jPatchCenter-dm:jPatchCenter+dm+1][i][j] = selectPatch[i][j]
